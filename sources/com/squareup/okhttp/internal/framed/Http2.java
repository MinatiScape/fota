    ...
    static final class Reader implements FrameReader {
        ...
        private void readData(FrameReader.Handler handler, int i, byte b2, int i2) throws IOException {
            ...
            if (!((b2 & 32) != 0)) {
                ...
                handler.data(z, i2, this.source, Http2.lengthWithoutPadding(i, b2, readByte));
        ...
        private void readHeaders(FrameReader.Handler handler, int i, byte b2, int i2) throws IOException {
            if (i2 != 0) {
                ...
                handler.headers(false, z, i2, -1, readHeaderBlock(Http2.lengthWithoutPadding(i, b2, readByte), readByte, b2, i2), HeadersMode.HTTP_20_HEADERS);
                return;
            }
        ...
        private void readPushPromise(FrameReader.Handler handler, int i, byte b2, int i2) throws IOException {
            if (i2 != 0) {
                short readByte = (b2 & 8) != 0 ? (short) (this.source.readByte() & 255) : (short) 0;
                handler.pushPromise(i2, this.source.readInt() & Integer.MAX_VALUE, readHeaderBlock(Http2.lengthWithoutPadding(i - 4, b2, readByte), readByte, b2, i2));
                return;
            }
        ...
        public void readConnectionPreface() throws IOException {
            ...
            if (Http2.logger.isLoggable(Level.FINE)) {
                Http2.logger.fine(String.format("<< CONNECTION %s", e.hex()));
            }
    ...
    static final class Writer implements FrameWriter {
        ...
        public synchronized void pushPromise(int i, int i2, List<Header> list) throws IOException {
            if (!this.closed) {
                ...
                int min = (int) Math.min(this.maxFrameSize - 4, p);
                long j = min;
                frameHeader(i, min + 4, Http2.TYPE_PUSH_PROMISE, p == j ? (byte) 4 : (byte) 0);
        ...
        void headers(boolean z, int i, List<Header> list) throws IOException {
            if (!this.closed) {
                this.hpackWriter.writeHeaders(list);
                long p = this.hpackBuffer.p();
                int min = (int) Math.min(this.maxFrameSize, p);
                long j = min;
